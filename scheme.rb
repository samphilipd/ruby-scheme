...
class Scheme
  ...
  def parse(program)
    case @reader_type
    when :recursive
      read_from_tokens_recursive(tokenize(program))
    when :iterative
      read_from_tokens_iterative(tokenize(program))
    end
  end
  ...
  # Works using recursive descent and is purely functional with no mutable state
  # Elegant but probably slower than the iterative version below and will fail
  # if we exceed the stack depth in the Ruby interpreter
  # => [:begin, [:define, :r, 10], [:*, :pi, [:*, :r, :r]]]
  def read_from_tokens_recursive(tokens, tree = [])
    ...
    if token.nil?
      ...
      new_sexp, tokens_remaining = read_from_tokens_recursive(next_tokens, [])
      read_from_tokens_recursive(tokens_remaining, tree + [new_sexp])
    elsif token == ')'
      [tree, next_tokens]
    else
      read_from_tokens_recursive(next_tokens, tree + [atom(token)])
    end
  end

  # An iterative (uglier but probably faster) approach to building a syntax tree
  # => [:begin, [:define, :r, 10], [:*, :pi, [:*, :r, :r]]]
